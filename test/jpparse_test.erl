...
test_parse(N, ShowParseTree, [{Test, Target} | Tests]) ->
  ...
  if
    ShowParseTree -> ?debugFmt("~p", [PTree]);
    true -> ok
  end,
  FoldTest =
    case jpparse_fold:string(PTree) of
      {ok, Ft} -> Ft;
...
t_parse(Tokens) ->
  case jpparse:parse(Tokens) of
    {ok, PTree} -> {ok, PTree};
    {error, {Line, Module, Message}} -> {error, {Line, lists:flatten(Module:format_error(Message))}}
  end.
