...
string(Pt) -> {ok, list_to_binary(foldbu(fun stringfun/3, [], Pt))}.

stringfun(_Depth, {'fun', _, Args} = _Pt, Stk) ->
  {PArgs, [A | Rest]} = lists:split(length(Args), Stk),
  [string:join([A, "(", string:join(lists:reverse(PArgs), ","), ")"], "") | Rest];
...
stringfun(_Depth, {Op, _, Args} = _Pt, Stk) when Op =:= '[]'; Op =:= '{}' ->
  ...
  [string:join([A, Lb, string:join(lists:reverse(PArgs), ","), Rb], "") | Rest];

stringfun(_Depth, {O, _, _} = _Pt, Stk) when O =:= ':'; O =:= '::'; O =:= '#' ->
  [B, A | Rest] = Stk,
  [string:join([A, atom_to_list(O), B], "") | Rest];

stringfun(_Depth, Pt, Stk) when is_binary(Pt) -> [binary_to_list(Pt) | Stk];
stringfun(_Depth, Pt, Stk) when is_integer(Pt) -> [integer_to_list(Pt) | Stk].
...
%  in a Top-Down walk
-spec foldtd(
  Function :: fun((Depth :: integer(), SubParseTree :: any(), AccIn :: any()) -> AccOut :: any()),
  Acc :: any(),
  ParseTree :: tuple()
) ->
  any().
foldtd(Fun, Acc, Pt) when is_function(Fun, 3) -> fold_i({td, Fun}, Acc, Pt).
...
-spec foldbu(
  ...
  any().
foldbu(Fun, Acc, Pt) when is_function(Fun, 3) -> fold_i({bu, Fun}, Acc, Pt).
...
fold_i({T, Fun}, Acc, {Op, L, R}, Lvl) when ((Op =:= '{}') orelse (Op =:= '[]')) andalso is_list(R) ->
  Acc1 = ?TD(Lvl, {Op, L, R}, Acc),
  Acc2 = fold_i({T, Fun}, Acc1, L, Lvl + 1),
  Acc3 = lists:foldl(fun (Ri, Acci) -> fold_i({T, Fun}, Acci, Ri, Lvl + 1) end, Acc2, R),
  ?BU(Lvl, {Op, L, R}, Acc3);
...
fold_i({T, Fun}, Acc, {'fun', Fn, Args}, Lvl) when is_binary(Fn), is_list(Args) ->
  ...
  Acc3 = lists:foldl(fun (Arg, Acci) -> fold_i({T, Fun}, Acci, Arg, Lvl + 1) end, Acc2, Args),
...
fold_i(_, _Acc, Pt, _) -> exit({unsupported, Pt}).
